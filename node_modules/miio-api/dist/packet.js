...
class Packet {
    ...
    static fromBuffer(buf) {
        ...
        if (magic !== Packet.MAGIC) {
            throw new errors_1.ProtocolError(`Invalid magic: 0x${magic.toString(16)}}`);
        }
        const size = buf.readUInt16BE(Packet.OFFSETS.SIZE);
        if (size !== buf.byteLength) {
            throw new errors_1.ProtocolError(`Invalid packet size, expected ${size} got ${buf.byteLength}`);
        }
        const unknown = buf.readUInt32BE(Packet.OFFSETS.UNKNOWN);
        if (unknown !== 0 && unknown !== 0xffffffff) {
            throw new errors_1.ProtocolError(`Invalid unknown: 0x${unknown.toString(16)}`);
        }
...
Packet.HEADER_SIZE = 32;
Packet.CHECKSUM_SIZE = 16;
